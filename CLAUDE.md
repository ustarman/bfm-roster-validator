...
## 확정된 업무 규칙 (2026-08-12, James 검증 후)
- **브레이크는 듀티 길이에 따라 30분 또는 60분.** 근무시간 = (Finish − Start) − 브레이크.
  - span ≤ 6h00 → 0분 / span ≤ 11h30 → 30분 / span > 11h30 → 60분
  - 기준 자료는 `New PAYS Roster Spreadsheet` (Downloads). 실제 linehaul 듀티 157셀 중
    **148셀이 이 규칙과 정확히 일치**하고, 불일치 9건은 전부 엑셀이 같은 span을 다른 값으로
    계산해 놓은 자기모순 셀이다(예: 13h45가 52건은 12.75인데 5건만 13.25).
  - 11h30 경계는 임의값이 아니라 규정에서 나온다: 30분 브레이크로 11h30을 넘기면 근무가
    11시간을 초과해 "12시간 내 최대 11시간" 규칙에 걸린다.
  - **경계에서 근무시간이 역전되는 건 의도된 것이다.** 11h30 span은 근무 11h인데 11h40 span은
    근무 10h40 — 브레이크가 30분에서 60분으로 늘기 때문. 이걸 매끄럽게 펴면 로스터에서 가장
    많은 듀티(BPF-C-BPF, 11h40)의 근무시간을 과다계상해 허위 위반이 난다. 테스트로 고정해뒀다.
  - 이 값들은 규정이 요구하는 시프트 내 휴식(6¼h→15분, 9h→30분, 12h→60분)을 모두 충족하므로
    그 세 규칙은 따로 검사하지 않는다.
